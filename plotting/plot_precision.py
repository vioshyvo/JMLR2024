...
def plot_recalls2(df, linewidth, linestyle, color, algo, is_first=True, plot=True):
    print(algo, ':')
    df = pareto_frontier(df, 'recall', 'precision', False)
    ...
    df.insert(1, 'precision', df.pop('precision'))
    df.index = [''] * len(df)
    print(df)
    print()
    x = df['recall']
...
algo_list = [['rf-rp', 'rp', 'rp-lookup'],
            ['rf-kd', 'kd', 'kd-lookup'],
            ['rf-pca', 'pca', 'pca-lookup']]
algonames = {'rf-rp': 'Natural classifier', 'rp-knn': r'Natural classifier, $\tau=0$',
             'rp' : 'Voting search',
             'rp-lookup' : r'Lookup search'}
...
def main(respath='article_results', k=10):
    outfname = 'precision-k' + str(k) + '.pdf'
    ...
    for j, dataset in enumerate(datasets):
        ...
        for u, algos in enumerate(algo_list):
            for i, algo in enumerate(algos):
                ...
                df['k_found'] = df['recall'] * df['k']
                df['precision'] = df['k_found'] / df['cs_size']
    ...
    plt.subplots_adjust(hspace=0.15) # 0.12
